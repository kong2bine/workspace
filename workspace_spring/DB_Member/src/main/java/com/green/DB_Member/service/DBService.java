...
public interface DBService {
    ...
    //전달받은 회원번호를 지닌 회원번호, 이름, 나이, 주소를 조회
    MemberVO select4(int memNum);

    //모든 회원의 회원번호, 이름, 나이, 주소를 조회
    List<MemberVO> select5();

    //delete, update, insert 리턴 타입 : void, int(조회된 데이터x, 영향을 받은 갯수가 출력됨)
