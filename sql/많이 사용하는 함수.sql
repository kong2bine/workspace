...
-- 문자열 나열 함수
SELECT 'JAVA', 'HELLO', CONCAT('JVAV', 'HELLO','PYTHON');
SELECT BOARD_NUM, TITLE, CONCAT(BOARD_NUM, '_', TITLE) AS BOARD_INFO
FROM board;

-- 문자열의 길이를 판단하는 함수
-- 컬럼명 입력시 해당 컬럼명의 길이를 알려줌
...
-- 세번째 : 추출 개수
SELECT 'ABCDEFG'
	, SUBSTRING('ABCDEFG',3) -- 3번 째, C부터 시작해서 문자열 추출
	, SUBSTRING('ABCDEFG',3,2) -- 3번 째부터 2개
...
-- 문자열 치환 함수(공백을 내가 입력한 값으로 변경해주는 것, 아래의 경우 _으로 채운거임)
SELECT '010 1111 2222',  REPLACE('010 1111 2222', ' ', '_');
...
-- 모든 사원의 이름과 급여의 합 조회 <- 모든 사원? 불가능함ㅋ
