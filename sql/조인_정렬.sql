...
-- 조인 : 두개 이상의 테이블을 동시에 조회 
-- 조인 조건(중요) : 두 테이블이 공통으로 지니는 컬럼의 값이 같은 데이터만 조회

-- 사원번호, 사원명, 부서명 조회
-- ㄱ이거 EMPNO, ENAME, DEPTNO 이렇게 약식으로 써도ㅇ but, WHERE절의 테이블명 생략 불가
...
-- 사원번호, 사원명, 직급, 부서번호, 부서명, 부서가 위치한 지역 조회
