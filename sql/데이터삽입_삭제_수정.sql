CREATE TABLE TEST_MEMBER(
	MEM_NUM INT PRIMARY KEY -- (기본키)  중복 불가, NULL 불가 
	, MEM_NAME VARCHAR(50)
	, MEM_AGE INT
	, MEM_ADDR VARCHAR(50) 
);
...
-- 데이터베이스는 데이터가 잘못되는 것을 엄격히 금지하기 떄문에 
-- 데이터의 변화가 생기는 경우, 해당 변화가 잘못이 없는지 한 번 확인할 기회를 줌
-- 변화된 데이터가 잘못되지 않았으면 COMMIT; 명령어로 데이터 변화를 확정지음
-- 잘못된 변화가 있을 때 변화를 취소시키고 싶다면 ROLLBACK; 명령어 실행 
